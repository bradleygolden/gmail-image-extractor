...
The user has the option to select various images through Gmail Image Extractor’s front-end interface. After all images selected for removal have been verified by the user, corresponding unique image id’s and message id’s are sent to the back-end. The back-end sorts each message into a list of attachments and then removes each selected attachment where the message id and image id match the user’s selection. The removal of each message is accomplished through the pygmail module.
